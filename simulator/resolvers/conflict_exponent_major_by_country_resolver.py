...
class ConflictExponentMajorByCountryResolver(ResolverBase[dict[str, float]]):

    variable = VariableEnum.conflict_exponent_major_by_country
    dependencies = [
    ...
    def resolve(cls, current_values: dict[VariableEnum, Any]) -> dict[str, float]:
        ...
        for country in current_values[VariableEnum.previous_logs_major_conflict_by_country].keys():
            ...
            for var, val in tentative_variable_values.items():
                if 'previous_neighborhood_has_major' in var.value:
                    var = VariableEnum(var.value.replace('previous_neighborhood_has_major', 'previous_neighborhood_has'))
            ...
            exponents_by_country[country] = dot_product(major_coeffs_by_variable, major_variable_values)
