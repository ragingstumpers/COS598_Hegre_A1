...
class CurrentOilTimesPreviousYearWasMajorByCountryResolver(ResolverBase[dict[str, float]]):

    variable = VariableEnum.current_oil_times_previous_year_was_major_by_country
    dependencies = [VariableEnum.current_oil_level_by_country, VariableEnum.previous_year_was_major_by_country]
    ...
    def resolve(cls, current_values: dict[VariableEnum, Any]) -> dict[str, int]:
        ...
        return {
            country: current_oil_levels_by_country[country] * previous_year_was_major_by_country[country]
